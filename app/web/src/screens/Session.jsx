  ...
  return (
    <Dojo rail={
      <RailMedia stem="session-route">
        <div className="font-serif text-lg text-washi">
          {isRest ? 'Rest day' : <span className="capitalize">{p.session_type}</span>}
        </div>
        <div className="mt-1 font-mono text-[11px] text-washi/60">
          {p.distance_km ? `${p.distance_km} km` : 'No session today'}
        </div>
      </RailMedia>
    }>
      ...
      <div className="mt-6">
        ...
        <Row label="Heart rate cap" value={p.hr_cap != null ? `${p.hr_cap} bpm` : '—'} />
